...
var makeAPIError = require('./errors').makeAPIError;
...
var hasError = function(response) {
  return (response.code && response.code !== SUCCESS) ||
...
var handlePayoneerResponse = function(callback, error, response, body) {
  ...
  parseXML(body, parseConfig, function(error, result) {
    if (error) return callback(error);
    if (hasError(result)) {
      return callback(makeAPIError(result));
    }
...
Payoneer.prototype.requestPayment = function(options, callback) {
  ...
  data = {
    ...
    p9: util.formatDate(options.date)
  };
